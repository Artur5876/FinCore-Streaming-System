...
namespace sql {
    // === Instruments ========================
        ...
        //========================================================
        //formatting helpers  (anonymous namespace — not part of API)
        //========================================================
        namespace {
        ...
        public:
            ...
            PGconn* acquire() {
                //lease a connection, block if none available
                std::unique_lock lk(mu_);
        ...
        //The single NON-TEMPLATE method that owns all pool + libpq
        ...
        // Copy and insert data efficiently all at once
        // A huge bulk of data through string_view(no copy);
            ...
            std::string_view   tsv) const
        {
            ...
            pool_->release(conn);                   //pool release
        ...
        //Column helpers for data conversion from PQserver result
        ...
        std::string Repository::to_pg_ts(TimePoint tp) {
            ...
            gmtime_r(&tt, &utc);
#endif
            return fmt::format("{:02d}-{:02d}-{:02d} {:02d}:{:02d}:{:02d}.{:06d}+00",
                utc.tm_mday, utc.tm_mon+1, utc.tm_year+1900,
                utc.tm_hour, utc.tm_min, utc.tm_sec, static_cast<long>(uk));
        }
